(function(window, undefined) {
		...
		init: function(selector) {
			...
			/*
				...
				2.字符串
					...
					选测器：会将找到的所有元素储存到jquery对象中并返回
				3.数组：
					会将数组中储存的元素依次存储到jquery对象中并返回
				4.除上述类型以外的：
					会将传入的数据储存到jquery对象中并返回
			*/
			// 0.去除字符串两端的空格
			...
			//4.数组
				...
				// 将数组转换为真数组(无论真伪数组都先转真数组，这样无需判断数组真伪优化4.1+4.2操作)
	...
	njQuery.prototype.extend({
		...
		inserAfter:function(sele){
			...
			$.each($target,function(key,value){
				var parent=value.parentNode;
				var next=$.get_nextsibling(value);
				// 2.便利取出所有的元素
				$this.each(function(k,v){
					// 判断当前时候是第0个指定
					if(key===0){
